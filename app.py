...
def load_model_4(model_dir, retriever_path):
    ...
    # Automatically picks up config.json, tokenizer.json, and tokenizer_config.json
    tokenizer = AutoTokenizer.from_pretrained(model_dir, use_fast=True)
    model = AutoModelForMultipleChoice.from_pretrained(model_dir).to(device)
    model.eval()
    
    # Load the retriever using joblib instead of pickle
...
def main():
    ...
    with col1:
        ...
        prompt = st.text_area("Prompt:", "Identify the correct statement: What is the effect generated by a spinning superconductor? among the listed options?", height=120)
        
        st.write("Options:")
        opt_a = st.text_input("Option A:", "An electric field, precisely aligned with the spin axis.")
        opt_b = st.text_input("Option B:", "A magnetic field, randomly aligned with the spin axis.")
        opt_c = st.text_input("Option C:", "A magnetic field, precisely aligned with the spin axis.")
        opt_d = st.text_input("Option D:", "A gravitational field, randomly aligned with the spin axis.")
        opt_e = st.text_input("Option E:", "A gravitational field, precisely aligned with the spin axis.")
